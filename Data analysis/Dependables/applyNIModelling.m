...
if isempty(initialGuess) || exist('freeParameters', 'var')
    ...
    if exist('freeParameters', 'var')
        ...
        %         tmpModelPar = modelPar(freeParameters,:);
        %         tmpinitialGuess = generateRandParameters(obj, tmpModelPar, numRepeats);
        %         initialGuess = [initialGuess(randi(size(initialGuess,1),numRepeats,1),:) tmpinitialGuess];
    else
